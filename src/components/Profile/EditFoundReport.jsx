...
const EditFoundReport = () => {
    ...
    const fetchReport = async () => {
      try {
        const { data } = await axios.get(
          `${server}/found/get-all-found-items`,
          { withCredentials: true }
        );
        const found = data.foundItems.find((r) => r._id === item_id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(
        `${server}/found/update-found-item/${item_id}`,
        report,
        { withCredentials: true }
      );
  ...
  const handleImageChange = async (index, file) => {
    try {
      ...
      const { data } = await axios.put(
        `${server}/found/update-found-item-image`,
        {
          itemId: item_id,
          imageIndex: index,
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              type="date"
              name="dateFound"
              value={report.dateFound ? report.dateFound.split("T")[0] : ""}
              onChange={handleChange}
              className="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#3a24db]"
              required
            />
